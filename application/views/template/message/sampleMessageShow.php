<?php
    $mysession = $_SESSION['MUserId'];
    $count = count($data);
    for($i = 0; $i < $count; $i++){
        if($data[$i]['sender_message_id'] == $mysession){
          $userid=    $data[$i]['sender_message_id'];
         $pic =  $this->Messagemodel->get_photo($userid);
        ?>
            <div id="receiver_msg_container">
                <div id="receiver_msg">
                        <p class="m-0" id="receiver_ptag"><?php echo $data[$i]['message'];?></p>
                </div>
                <div id="receiver_image" style="background-size: 100% 100%; background-image:url('<?php echo "uploads/photo/".$pic->photo;?>')"></div>
            </div>
        <?php
        }else{
