...
$('body').on('change | keyup', `[name="pro_code"]`, function(e) {
        ...
        $.post('<?=base_url()?>Billing/getProduct/', { pro_code: pro_code })
            .done(function(data) {
...
$(document).ready(function() {
    $('#orderForm').submit(function(e) {
        ...
        $.ajax({
            type: 'POST',
            url: '<?=base_url('Billing/ProductOrderSubmit');?>',
            data: formData,
            dataType: 'json', 
            success: function(response) {
                if (response.success) {
                    ...
                    .then((value) => {
                        switch (value) {
                            case 'pdf':
                                window.open('<?php echo base_url('normal-invoice/') ;?>'+response.invoice_no, '_blank');
                                break;
                            case 'printer':
                                window.open('<?php echo base_url('normal-invoice/') ;?>'+response.invoice_no, '_blank');
                                break;
