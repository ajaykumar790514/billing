...
<!-- onload="window.print()" -->
    ...
    echo $timestring = $now->format('d-m-Y');?></b>
       ...
       <hr>
       <?php 
            if($invoice[0]['customer_id'] !='0'){
                $billto = $invoice[0]['cus_address1'].' &nbsp;'.$invoice[0]['cus_address_city'].' <br> '. $invoice[0]['cus_address_state'].'&nbsp;'.$invoice[0]['cus_address_pincode'].'&nbsp;'.$invoice[0]['cus_address_country'];

                $shipto = $invoice[0]['cus_shipping_address'].' &nbsp;'.$invoice[0]['cus_shipping_city'].' <br> '. $invoice[0]['cus_shipping_state'].'&nbsp;'.$invoice[0]['cus_shipping_pincode'].'&nbsp;'.$invoice[0]['cus_shipping_country']  ;
                $name = $invoice[0]['cus_name'];
                $per_mobile = $invoice[0]['cus_permanant_number'];
                $alt_mobile = $invoice[0]['cus_alternate_number'];
                $email = $invoice[0]['cus_email'];
            }elseif($invoice[0]['supplier_id'] !='0')
            {
                $billto = $invoice[0]['sup_address'].' &nbsp;'.$invoice[0]['sup_city'].' <br> '. $invoice[0]['sup_state'].'&nbsp;'.$invoice[0]['sup_pincode'].'&nbsp;'.$invoice[0]['sup_country']  ;

                $shipto = $invoice[0]['sup_address'].' &nbsp;'.$invoice[0]['sup_city'].' <br> '. $invoice[0]['sup_state'].'&nbsp;'.$invoice[0]['sup_pincode'].'&nbsp;'.$invoice[0]['sup_country']  ;
                $name = $invoice[0]['sup_name'];
                $per_mobile = $invoice[0]['sup_permanant_number'];
                $alt_mobile = $invoice[0]['sup_alternate_number'];
                $email = $invoice[0]['sup_email'];
            }
        ...
        <tr>
            ...
            <p style="margin-top: -0.2rem;margin-bottom:0.8rem;font-family:  Arial, Helvetica, sans-serif;"><b><u>BILL TO</u></b></p>
                    <p style="font-family:  Arial, Helvetica, sans-serif;margin-bottom: 0.5rem;" >Name: <strong><?= $name ?></strong></p>
                    <p  style="font-family:  Arial, Helvetica, sans-serif;margin-bottom: 0.5rem;">Address: <?= $billto  ? $billto : '<strong>N/A</strong>'; ; ?> </p>
                   
                    <p  style="font-family:  Arial, Helvetica, sans-serif;margin-bottom: 0.5rem;"> Phone: <?=  $per_mobile ? $per_mobile : '<strong>N/A</strong>'; ?></p>
                    <p  style="font-family:  Arial, Helvetica, sans-serif;margin-bottom: 0.5rem;"> Alternate Phone: <?=  $alt_mobile ? $alt_mobile : '<strong>N/A</strong>'; ?></p>
                    <p  style="font-family:  Arial, Helvetica, sans-serif;margin-bottom: 0.5rem;">Email: <?= $email ?></p>
                    <p  style="font-family:  Arial, Helvetica, sans-serif;">GSTIN: <strong>N/A</strong></p>
            ...
            <p style="margin-top: -0.2rem;margin-bottom:0.8rem;font-family:  Arial, Helvetica, sans-serif;"><b><u>SHIP TO</u></b></p>
                    <p  style="font-family:  Arial, Helvetica, sans-serif;margin-bottom: 0.5rem;">Name: <strong><?= $name ?></strong></p>
                    <p  style="font-family:  Arial, Helvetica, sans-serif;margin-bottom: 0.5rem;">Address: <?= $shipto  ? $shipto : '<strong>N/A</strong>';  ?> </p>
       ...
       <table width="100%">
        ...
        <?php
          // discount
            ...
            $i=1;$TaxPrice=$Tax=$Price=$TaxValue=$PricePer=$totalamount=0;
            foreach($invoice as $row):
                $Tax = $Price= $TaxValue=0;
                $Price = $row['selling_rate'];
                $Tax = $row['pro_tax'];
                $TaxValue = (($Price*$Tax)/100);
                $TaxPrice = (($Price*$Tax)/100)*$row['qty'];
                $PricePer = $row['selling_rate']-$TaxValue;
            ...
            <tr>
                ...
                <td style="border:2px solid black;height:40px;width:80px;text-align:center;font-family:  Arial, Helvetica, sans-serif;">₹<?=$TaxPrice?></td>
                <td style="border-right:5px !important;border:2px solid black;height:40px;width:80px;text-align:center;font-family:  Arial, Helvetica, sans-serif;">₹<?= $row['product_amount']; ?></td>
            </tr>
		  <?php
            $i=$i+1;
            endforeach;
		...
		<table width="100%">
			...
			<tr>
                ...
                <th style="width:184px;border-top:3px !important;border-right:3px !important;border:2px solid black;text-align:right;font-family:  Arial, Helvetica, sans-serif;">₹<?= round($invoice[0]['amount']); ?></th>
			</tr>
		</table>
        <div id="total-amount">
		  	<p>Total Amount (in words) : <strong>INR <?= number_to_word($invoice[0]['amount']); ?></strong></p>
		</div>
        <hr>
